...
export default function RegisterPage() {
  ...
  function getStrength(): { label: string; color: string; width: string } {
    ...
    if (password.length < 6) return { label: "Weak", color: "bg-loss", width: "33%" };
    const hasUpper = /[A-Z]/.test(password);
    const hasNumber = /[0-9]/.test(password);
    const hasSpecial = /[^A-Za-z0-9]/.test(password);
    const score = [hasUpper, hasNumber, hasSpecial, password.length >= 10].filter(Boolean).length;
    if (score >= 3) return { label: "Strong", color: "bg-profit", width: "100%" };
  ...
  if (success) {
    return (
      <div className="text-center py-4">
        <div className="inline-flex items-center justify-center h-16 w-16 rounded-full bg-profit/10 mb-4">
          <svg className="h-8 w-8 text-profit" fill="none" viewBox="0 0 24 24" stroke="currentColor">
